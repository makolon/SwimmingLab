...
class DroneVelocityAction(ActionTerm):
    """Convert (v_x, v_y, v_z, yaw) commands into rotor forces via a Lee controller."""

    cfg: actions_cfg.DroneVelocityActionCfg
    _asset: Articulation  # provided by ActionTerm
    ...
    def apply_actions(self) -> torch.Tensor:
        ...
        tau = torch.where(target_throttle > self._rotor_throttle, self._tau_up, self._tau_down).clamp_(0.0, 1.0)
        self._rotor_throttle += tau * (target_throttle - self._rotor_throttle)
        ...
        z_axis_local = torch.tensor([0.0, 0.0, 1.0], device=self.device).expand_as(rotor_pos_w)
        ...
        # TODO: Fix this
        # self._asset.data.body_vel_w[0, self._base_body_ids, :]
        ...
        self._rotor_velocities = (
            self._rotor_throttle * self._directions * self._max_rotvel
        )
